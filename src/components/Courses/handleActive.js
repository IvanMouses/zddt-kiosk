...
export default function handleActive(e) {
  document
    .querySelectorAll(`.${classes.coursesShowMoreCardProgrammScheduleColumn}`)
    .forEach((column) => {
      if (e.target.closest("div")?.dataset.group === column.dataset.group) {
        column.classList.toggle(classes.active);
      }
    });
  document
  ...
  if (
    ...
    document
      .querySelectorAll(`.${classes.coursesShowMoreCardProgrammScheduleColumn}`)
      .forEach((column) => {
        column.classList.remove(classes.active);
      });
